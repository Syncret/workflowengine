...
public class DynamicInvoker {
	...
	public Map invoke(String serviceName, String portName,
		...
		//servercomment System.out.println("When createCall ||||  portName = " + portName+ "   operationName = " + operationName);
		...
		bindingEntry.getQName();
		if (parameters.returnParam != null) {
			QName returnType = org.apache.axis.wsdl.toJava.Utils
					.getXSIType(parameters.returnParam);
			//cyd add 解决arrayofstring的映射
			if (returnType.getLocalPart().equalsIgnoreCase("arrayofstring")) {
				
				call.setReturnType(returnType,String[].class);
			}
			//
		...
		// call.addParameter("ArrayOfString",
		// org.apache.axis.Constants.XSD_STRING,
		// ParameterMode.OUT);

		// QName qn=new QName("urn:BeanService","ArrayOfString");

		/*
		 * QName dtoqn = new QName("urn:BeanService","ArrayOfString"); Class
		 * dtocls = UndueStmtDTO.class; call.registerTypeMapping(dtocls, dtoqn,
		 * new org.apache.axis.encoding.ser.BeanSerializerFactory(dtocls,
		 * dtoqn), new
		 * org.apache.axis.encoding.ser.BeanDeserializerFactory(dtocls, dtoqn));
		 */
		Object ret = call.invoke(inputs.toArray());
		Map outputs = call.getOutputParams();
		HashMap map = new HashMap();
		
		List a=call.getOutputValues();
